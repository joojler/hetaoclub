...
{
	public $questions_list_total = 0;
	public $search_questions_total = 0;
	
	public function get_questions_list($page = 1, $per_page = 10, $sort = null, $topic_id = null, $category_id = null, $answer_count = null, $day = 30, $is_recommend = false, $is_offline = 0)
	{
	...
	public function get_offline_flag_by_question_id($question_id)
	{
		return $this->query_row('SELECT is_offline FROM ' . $this->get_table('question') . ' WHERE question_id = ' . intval($question_id));
	}
	...
	public function search_questions_list($page, $per_page, $keyword = null, $category_id = null, $start_date = null, $end_date = null, $answer_count_min = null, $answer_count_max = null, $user_name = null, $best_answer = false)
	{
		...
		if ($keyword)
		{
			...
			$where[] = "(MATCH(question_content_fulltext) AGAINST('" . $this->quote($this->model('search_fulltext')->encode_search_code($analysis_keyword)) . "' IN BOOLEAN MODE))";
		}
	 ...
	 */
	public function save_question($question_content, $question_detail, $published_uid, $anonymous = 0, $is_offline = '0', $ip_address = null)
	{
		if (!$ip_address)
		{
			$ip_address = fetch_ip();
	...
	{
		if ($question_related_list = AWS_APP::cache()->get('question_related_list_' . md5($question_content) . '_' . $limit))
		{
		...
		AWS_APP::cache()->set('question_related_list_' . md5($question_content) . '_' . $limit, $question_related_list, get_setting('cache_level_low'));
	...
	public function get_near_by_questions($longitude, $latitude, $uid, $limit = 10)
	{
		...
		if ($near_by_questions = $this->fetch_all('question', "`published_uid` != " . intval($uid) . " AND `latitude` > 0 AND `latitude` > " . $squares['BR']['latitude'] . " AND `latitude` < " . $squares['TL']['latitude'] . " AND `longitude` > " . $squares['TL']['longitude'] . " AND `longitude` < " . $squares['BR']['longitude'], 'add_time DESC'))
		{
			foreach ($near_by_questions AS $key => $val)
			{
				$near_by_uids[$val['published_uid']] = $val['published_uid'];
			}
			...
			foreach ($near_by_questions AS $key => $val)
			{
				...
				$near_by_questions[$key]['distance'] = $this->model('geo')->get_distance($longitude, $latitude, $val['longitude'], $val['latitude']);
			}
		}
